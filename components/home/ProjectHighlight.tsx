...
import { ShieldCheck, Trees, ArrowRight, FileText, Users, MapPin } from 'lucide-react';
...
const projects = [
   {
      ...
      description: "Premium villa plots for a grand life in Attibele–Jigala. Prime Location next to the proposed Swift City IT Hub.",
      ...
      details: [
         ...
         { label: "Status", value: "Ready to Register", statusColor: "text-green-400" },
   ...
   },
   {
      ...
      title: "The Meadows.",
      tagline: "The Development",
      description: "Premium gated community plots in Taliparamba, Kerala. A perfect blend of peace & connectivity in a secure environment.",
      ...
      details: [
         ...
         { label: "Status", value: "Ready to Build", statusColor: "text-green-400" },
...
const ProjectHighlight = () => {
   return (
      <section id="projects" className="py-20 px-6 max-w-[1400px] mx-auto space-y-32">
         {projects.map((project, index) => (
            <div key={project.id} className="relative">
               {/* --- PROJECT HEADER --- */}
               <div className="mb-10 flex flex-col md:flex-row justify-between items-end gap-6 border-b border-[#808080]/20 pb-6">
                  <div>
                     <span className="text-[#11123A] text-xs font-bold tracking-[0.2em] uppercase mb-2 block">{project.tagline}</span>
                     <h2 className="text-4xl md:text-5xl font-black text-[#171717] tracking-tight">{project.title}</h2>
                  </div>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-auto lg:min-h-[500px]">
                  ...
                  <div className="lg:col-span-8 rounded-2xl overflow-hidden relative group min-h-[350px] lg:min-h-full">
                     ...
                     <div className="absolute bottom-8 left-8 right-8 flex flex-col sm:flex-row justify-between items-end gap-4">
                        <div>
                           <h3 className="text-3xl font-bold text-[#ffffff] mb-2 drop-shadow-lg">Premium Living</h3>
                           <div className="flex items-center gap-4 text-[#ffffff]/90 text-sm font-bold">
                              {project.features.map((feature, fIndex) => (
                                 <span key={fIndex} className="flex items-center gap-1 bg-[#ffffff]/20 px-3 py-1 rounded-md backdrop-blur-sm">
                        ...
                        >
                           Explore Project <ArrowRight size={16} className="group-hover/btn:translate-x-1 transition-transform" />
                        </Link>
                     </div>
                  </div>
                  ...
                  <div className="lg:col-span-4 flex flex-col gap-6">
                     <div className="bg-[#11123A] rounded-2xl p-8 h-full flex flex-col justify-center shadow-lg relative overflow-hidden">
                        ...
                        <h4 className="text-[#ffffff] text-2xl font-bold mb-6 relative z-10">Key Details</h4>
                        <ul className="space-y-4 relative z-10">
                           {project.details.map((detail, dIndex) => (
                              <li key={dIndex} className={`flex justify-between ${dIndex !== project.details.length - 1 ? 'border-b border-[#ffffff]/10 pb-3' : 'pb-3'}`}>
                                 <span className="text-[#ffffff]/60 text-sm font-bold uppercase">{detail.label}</span>
                                 <span className={`text-[#ffffff] font-bold ${detail.statusColor || ''}`}>{detail.value}</span>
                              </li>
                           ))}
                        </ul>
                        {project.brochure && (
                     ...
                     <div className="bg-[#ffffff] border border-[#808080]/20 rounded-2xl p-6 flex items-center gap-4 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div>
                           <h4 className="font-bold text-[#171717]">{project.community}</h4>
                           <p className="text-[#808080] text-xs font-medium">{project.communityDesc}</p>
                        </div>
                     </div>
                  </div>
               </div>
         ...
         <div className="pt-16 text-center">
            ...
            >
               Explore All Projects <ArrowRight size={18} className="group-hover:translate-x-2 transition-transform" />
            </Link>
         </div>
      </section>
   );
